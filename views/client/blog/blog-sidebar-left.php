<?php include_once "views/client/layout/header.php" ?>


        <!-- page-title -->
        <div class="tf-page-title">
            <div class="container-full">
                <div class="row">
                    <div class="col-12">
                        <div class="heading text-center">Left Sidebar</div>
                        <ul class="breadcrumbs d-flex align-items-center justify-content-center">
                            <li>
                                <a href="index.html">Home</a>
                            </li>
                            <li>
                                <i class="icon-arrow-right"></i>
                            </li>
                            <li>
                                Blog Left Sidebar
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <!-- /page-title -->
       
        <!-- blog-grid-main -->
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="blog-sidebar-main">
                        <div class="tf-section-sidebar wrap-sidebar-mobile">
                            <div class="sidebar-item sidebar-categories">
                                <div class="sidebar-title">Blog categories</div>
                                <div class="sidebar-content">
                                    <ul>
                                        <li>
                                            <a href="#">Accessories<span>(9)</span></a>
                                        </li>
                                        <li>
                                            <a href="#">Bag<span>(9)</span></a>
                                        </li>
                                        <li>
                                            <a href="#">Cookware & Kitchen<span>(9)</span></a>
                                        </li>
                                        <li>
                                            <a href="#">Decor<span>(9)</span></a>
                                        </li>
                                        <li>
                                            <a href="#">Decorate<span>(9)</span></a>
                                        </li>
                                        <li>
                                            <a href="#">Denim<span>(9)</span></a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="sidebar-item sidebar-post">
                                <div class="sidebar-title">Recent Post</div>
                                <div class="sidebar-content">
                                    <ul>
                                        <li>
                                            <div class="blog-article-item style-sidebar">
                                                <div class="article-thumb">
                                                    <a href="blog-detail.html">
                                                        <img src="images/shop/article/sidebar-1.jpg" alt="">
                                                    </a>
                                                </div>
                                                <div class="article-content">
                                                    <div class="article-label">
                                                        <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Accessories</a>
                                                    </div>
                                                    <div class="article-title">
                                                        <a href="blog-detail.html" class="">Effortlessly Blends The Carefree Style</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="blog-article-item style-sidebar">
                                                <div class="article-thumb">
                                                    <a href="blog-detail.html">
                                                        <img src="images/shop/article/sidebar-2.jpg" alt="">
                                                    </a>
                                                </div>
                                                <div class="article-content">
                                                    <div class="article-label">
                                                        <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Accessories</a>
                                                    </div>
                                                    <div class="article-title">
                                                        <a href="blog-detail.html" class="">The Limited Edition Club des Sports Courchevel</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="blog-article-item style-sidebar">
                                                <div class="article-thumb">
                                                    <a href="blog-detail.html">
                                                        <img src="images/shop/article/sidebar-3.jpg" alt="">
                                                    </a>
                                                </div>
                                                <div class="article-content">
                                                    <div class="article-label">
                                                        <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Fashion</a>
                                                    </div>
                                                    <div class="article-title">
                                                        <a href="blog-detail.html" class="">Christine Is A True Style Icon</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="sidebar-item sidebar-tags">
                                <div class="sidebar-title">Blog tags</div>
                                <div class="sidebar-content">
                                    <ul class="tags-lists">
                                        <li>
                                            <a href="#" class="tags-item">Bags</a>
                                        </li>
                                        <li>
                                            <a href="#" class="tags-item">Fashion</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="sidebar-item sidebar-instagram">
                                <div class="sidebar-title">Instagram</div>
                                <div class="sidebar-content">
                                    <ul>
                                        <li>
                                            <img src="images/shop/file/img-1.jpg" alt="">
                                        </li>
                                        <li>
                                            <img src="images/shop/file/img-2.jpg" alt="">
                                        </li>
                                        <li>
                                            <img src="images/shop/file/img-3.jpg" alt="">
                                        </li>
                                        <li>
                                            <img src="images/shop/file/img-4.jpg" alt="">
                                        </li>
                                        <li>
                                            <img src="images/shop/file/img-5.jpg" alt="">
                                        </li>
                                        <li>
                                            <img src="images/shop/file/img-6.png" alt="">
                                        </li>
            
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="list-blog">
                            <div class="blog-article-item">
                                <div class="article-thumb">
                                    <a href="blog-detail.html">
                                        <img class="lazyload" data-src="images/shop/article/image-1.jpg" src="images/shop/article/image-1.jpg" alt="img-blog">
                                    </a>
                                    <div class="article-label">
                                        <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Tech</a>
                                    </div>
                                </div>
                                <div class="article-content">
                                    <div class="article-title">
                                        <a href="blog-detail.html" class="">The Limited Edition Club des Sports Courchevel</a>
                                    </div>
                                    <div class="article-btn">
                                        <a href="blog-detail.html" class="tf-btn btn-line fw-6">Read more<i class="icon icon-arrow1-top-left"></i></a>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="blog-article-item">
                                        <div class="article-thumb">
                                            <a href="blog-detail.html">
                                                <img class="lazyload" data-src="images/shop/article/image-2.jpg" src="images/shop/article/image-2.jpg" alt="">
                                            </a>
                                            <div class="article-label">
                                                <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Tech</a>
                                            </div>
                                        </div>
                                        <div class="article-content">
                                            <div class="article-title">
                                                <a href="blog-detail.html" class="">The Variety Of Styles And Prices Are Endless</a>
                                            </div>
                                            <div class="article-btn">
                                                <a href="blog-detail.html" class="tf-btn btn-line fw-6">Read more<i class="icon icon-arrow1-top-left"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="blog-article-item">
                                        <div class="article-thumb">
                                            <a href="blog-detail.html">
                                                <img class="lazyload" data-src="images/shop/article/sidebar-1.jpg" src="images/shop/article/sidebar-1.jpg" alt="">
                                            </a>
                                            <div class="article-label">
                                                <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Tech</a>
                                            </div>
                                        </div>
                                        <div class="article-content">
                                            <div class="article-title">
                                                <a href="blog-detail.html" class="">Effortlessly Blends The Carefree Style</a>
                                            </div>
                                            <div class="article-btn">
                                                <a href="blog-detail.html" class="tf-btn btn-line fw-6">Read more<i class="icon icon-arrow1-top-left"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="blog-article-item">
                                <div class="article-thumb">
                                    <a href="blog-detail.html">
                                        <img class="lazyload" data-src="images/shop/article/sidebar-3.jpg" src="images/shop/article/sidebar-3.jpg" alt="">
                                    </a>
                                    <div class="article-label">
                                        <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Tech</a>
                                    </div>
                                </div>
                                <div class="article-content">
                                    <div class="article-title">
                                        <a href="blog-detail.html" class="">Christine Is A True Style Icon</a>
                                    </div>
                                    <div class="article-btn">
                                        <a href="blog-detail.html" class="tf-btn btn-line fw-6">Read more<i class="icon icon-arrow1-top-left"></i></a>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="blog-article-item">
                                        <div class="article-thumb">
                                            <a href="blog-detail.html">
                                                <img class="lazyload" data-src="images/shop/article/image-3.jpg" src="images/shop/article/image-3.jpg" alt="">
                                            </a>
                                            <div class="article-label">
                                                <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Tech</a>
                                            </div>
                                        </div>
                                        <div class="article-content">
                                            <div class="article-title">
                                                <a href="blog-detail.html" class="">Hello Fashion by Colombian-American</a>
                                            </div>
                                            <div class="article-btn">
                                                <a href="blog-detail.html" class="tf-btn btn-line fw-6">Read more<i class="icon icon-arrow1-top-left"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="blog-article-item">
                                        <div class="article-thumb">
                                            <a href="blog-detail.html">
                                                <img class="lazyload" data-src="images/shop/article/image-4.jpg" src="images/shop/article/image-4.jpg" alt="">
                                            </a>
                                            <div class="article-label">
                                                <a href="blog-detail.html" class="tf-btn btn-sm radius-3 btn-fill animate-hover-btn">Tech</a>
                                            </div>
                                        </div>
                                        <div class="article-content">
                                            <div class="article-title">
                                                <a href="blog-detail.html" class="">An Exclusive Clothing Collaboration</a>
                                            </div>
                                            <div class="article-btn">
                                                <a href="blog-detail.html" class="tf-btn btn-line fw-6">Read more<i class="icon icon-arrow1-top-left"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <ul class="wg-pagination">
                                <li class="active">
                                    <div class="pagination-item">1</div>
                                </li>
                                <li>
                                    <a href="#" class="pagination-item animate-hover-btn">2</a>
                                </li>
                                <li>
                                    <a href="#" class="pagination-item animate-hover-btn">3</a>
                                </li>
                                <li>
                                    <a href="#" class="pagination-item animate-hover-btn"><i class="icon icon-arrow-right"></i></a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="btn-sidebar-mobile left">
            <button data-bs-toggle="offcanvas" data-bs-target="#sidebarmobile" aria-controls="offcanvas"><i class="icon-open"></i></button>
        </div>
        <!-- /blog-grid-main -->

        <?php include_once "views/client/layout/footer.php" ?>
