...
<body class="preload-wrapper">
    ...
    <tbody>
        <?php foreach ($cartItems as $item): ?>
            <tr>
                ...
                <td>
                    <img src="<?= htmlspecialchars($item['product_image']) ?>" alt="<?= htmlspecialchars($item['product_name']) ?>" style="width: 50px; height: 50px; object-fit: cover;">
                </td>
                <td>
                    <!-- Form cập nhật số lượng -->
                    <form action="index.php?ctl=update_cart" method="POST">
    ...
    <button type="submit">Cập nhật</button>
                ...
                <td><?= number_format($item['product_price'] * $item['quantity'], 2, ',', '.') ?> VNĐ</td>
                <td>
                    <a href="index.php?action=viewcart&account_id=<?= $item['account_id'] ?>&remove_cart_id=<?= $item['cart_id'] ?>">Xóa</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
    ...
    <div class="cart-total">
        <strong>Tổng giá trị giỏ hàng: <?= number_format($total, 0, ',', '.') ?> VND</strong>
    </div>

    <!-- Liên kết thanh toán -->
    <a href="index.php?action=checkout&account_id=<?= $account_id ?>" class="checkout-btn">Tiến hành thanh toán</a>
